...
import sys, json, re, httpx, os, argparse
import pprint as pp
...
def checkResponse(request: str):
    ...
    if response.status_code == 200 or (
        response.status_code == 204 and request == "DELETE"
    ):
...
def getToken():
    ...
    response = httpx.post(url=request_endpoint, json=request_data)
    checkResponse("POST")
...
def getEndpointData() -> tuple:
    ...
    response = httpx.get(url=request_endpoint, headers=request_header)
    checkResponse("GET")
...
def isStackDeployed() -> str:
    ...
    # STACK_NAME = "pontusc-test-rest-main"
    # Setup
...
def deploy():
    ...
    if stack_id:  # Not empty string, stack exists
        ...
        # Update stack
        response = httpx.put(
            url=request_endpoint, headers=request_header, json=request_json
        )
        checkResponse("PUT")
...
def teardown():
    ...
    if stack_id:  # Not empty string, is deployed
        ...
        # Remove stack
        response = httpx.delete(url=request_endpoint, headers=request_header)
